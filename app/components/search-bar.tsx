...
export function SearchBar({ profile }: props) {
  ...
  const handleSort = (
    ...
    setSortData((form) => ({ ...form, [field]: event.target.value }));
  };

  return (
    <form className="w-full px-10 flex items-center gap-x-4 border-b-4 border-b-blue-900 border-opacity-30 h-20">
      ...
      <SelectBox
        ...
        options={sortOptions}
        onChange={(e) => handleSort(e, "sortBy")}
      />
      <button
        type="submit"
      ...
      <UserCircle
        className="h-14 w-14 transition duration-300 ease-in-out hover:scale hover:border-2 hover:border-yellow-300"
        profile={profile}
